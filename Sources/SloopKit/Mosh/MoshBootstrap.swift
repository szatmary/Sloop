...
public struct MoshBootstrap: Equatable {
    ...
    /// The address the SSH leg actually reached, as a numeric string, or nil
    /// when it isn't known.
    ///
    /// Mosh resolves its host argument with `AI_NUMERICHOST`, so it must be
    /// handed an address rather than a name — and it has to be *this* address.
    /// Resolving the hostname a second time can land on a different machine
    /// behind round-robin DNS, or pick the A record where the SSH leg used the
    /// AAAA, and `mosh-server` is only running on the one just spoken to.
    ///
    /// Filled in by `MoshBootstrapper` from the runner that ran the bootstrap,
    /// which is the only thing that knows.
    public var serverAddress: String?

    public init(udpPort: Int, key: String, serverAddress: String? = nil) {
