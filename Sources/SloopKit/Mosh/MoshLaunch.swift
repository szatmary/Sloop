...
public final class MoshBootstrapper {
    ...
    public func bootstrap(completion: @escaping (MoshBootstrapResult) -> Void) {
        ...
        runner.run(MoshServer.script(extraCommands: extras)) { [runner] result in
            switch result {
            case .success(let output):
                ...
                let (banner, outputs) = MarkedCommandBatch.split(output.stdoutText,
                                                                 count: extras.count)
                var startup = MoshServer.interpret(banner + "\n" + output.stderrText)
                // The SSP leg has to reach the machine that just started
                // `mosh-server`, and only the runner knows which one that was.
                if case .connect(var bootstrap) = startup, let address = runner.peerAddress {
                    bootstrap.serverAddress = address
                    startup = .connect(bootstrap)
                }
                completion(MoshBootstrapResult(startup: startup, extraOutputs: outputs))
