...
public protocol AccessTokenStore: AnyObject, Sendable {
    /// The stored JWT, or nil if this hostname has none. Throws when the store
    /// cannot be read: a keychain that refuses the read is not a hostname
    /// nobody has signed in to, and reporting it as one sends the user through
    /// a browser login that cannot fix it.
...
public extension AccessTokenStore {
    ...
    func validToken(for hostname: String) throws -> AccessToken? {
        guard let raw = try rawToken(for: hostname) else { return nil }
