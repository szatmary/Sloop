...
public enum KeyEncoder {
    ...
    /// Encode a typed character with modifiers.
    ///
    /// Control masks with `& 0x1F`, but only over `@`…`_`, where a control
    /// form exists (so Ctrl-C → 0x03, Ctrl-[ → ESC). Digits and a handful of
    /// punctuation have their own xterm mappings; everything else is sent
    /// unchanged. Option prefixes ESC.
    public static func bytes(for character: Character, modifiers: KeyModifiers = []) -> [UInt8] {
        ...
        if modifiers.contains(.control), let ascii = character.asciiValue {
            ...
            if let digit = Self.controlDigit(ascii) {
                out = [digit]
            } else if let punctuation = Self.controlPunctuation(ascii) {
                out = [punctuation]
            } else {
                // Upper-case ASCII letters before masking so 'c' and 'C' both → 0x03.
                let base = (ascii >= 0x61 && ascii <= 0x7a) ? ascii - 0x20 : ascii
                // Only `@`…`_` has a control form. Masking the rest invents
                // one: `-` became 0x0d — Return, executing the line — and `;`
                // became Escape. Anything else is sent as itself, which is what
                // xterm and Terminal.app do.
                out = (0x40...0x5f).contains(base)
                    ? [base & 0x1f]
    ...
    /// The characters outside `@`…`_` that still have a control form.
    ///
    /// Space is the one the mask happened to get right (0x20 & 0x1F is 0) and
    /// is kept explicit here rather than relying on that. The other three are
    /// xterm's: `⌃?` is DEL, and both `⌃/` and `⌃-` reach US — readline's undo,
    /// which is what a hand reaching for `⌃-` on a compact keyboard wants.
    private static func controlPunctuation(_ ascii: UInt8) -> UInt8? {
        switch ascii {
        case 0x20: return 0x00        // Ctrl-Space → NUL
        case 0x3f: return 0x7f        // Ctrl-? → DEL
        case 0x2f, 0x2d: return 0x1f  // Ctrl-/ and Ctrl-- → US
        default: return nil
        }
    }
