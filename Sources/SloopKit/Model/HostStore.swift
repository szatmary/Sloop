...
public final class HostStore {
    ...
    ///
    /// Throws if the write fails, and changes nothing when it does. This used
    /// to be `try? save()`, which meant the caller showed the host as saved
    /// and the next launch showed it gone — the silent loss the quarantine
    /// design above exists to prevent, reintroduced one line later.
    ...
    public func remove(_ host: SSHHost) throws {
        try applyAndSave {
    ...
    /// Applies `change`, writes, and rolls back if the write fails.
    ///
    /// The rollback is what makes the error honest. Without it a store whose
    /// save failed keeps the change in memory, and the *next* save — of some
    /// unrelated edit that succeeds — writes it out too, turning a reported
    /// failure into a silent success minutes later.
    private func applyAndSave(_ change: () -> Void) throws {
        let previousHosts = hosts
        let previousUnparsed = unparsedRecords
        change()
        do {
            try save()
        } catch {
            hosts = previousHosts
            unparsedRecords = previousUnparsed
            throw error
        }
    }
