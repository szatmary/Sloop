...
public struct SSHHost: Identifiable, Codable, Hashable {
    ...
    ///
    /// Off unless asked for, including for hosts saved before the feature
    /// existed. A feature that writes what you type to a file is not one to
    /// switch on for someone; the decode default matters as much as the
    /// property's, since a host written by an older build carries no key at all.
    public var suggestions: Bool = false
