...
public protocol CommandRunner {
    ...
    /// The address the last `run` reached, as a numeric string.
    ///
    /// Only meaningful once a run has completed, and only a real SSH runner
    /// can answer — hence the default. It exists for the Mosh bootstrap, which
    /// must send its UDP packets to the same machine the SSH leg authenticated
    /// against rather than to whatever a second DNS lookup returns.
