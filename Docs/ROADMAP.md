...
- ~~Command suggestions~~ → DONE, in the form that needs no model: a bar above
  ...
  command rather than the hundredth. Off by default, per host — genuinely so
  since 2026-09-10, when the decode default was fixed to match this sentence.
  `CommandLineTracker` reconstructs the line from the bytes Sloop *sends*, and
  watches the bytes the host returns to decide whether it may keep it: a line
  is recorded only once the host has echoed every character of it back. That is
  the whole password defence, and it names no prompts — `sudo`, `read -s`,
  `mysql -p` and the ones not yet invented share only that they turn echo off,
  so what you type does not come back. A pasted block is finished before any
  echo arrives, so pastes run without being learned, which is the wanted answer
  for the `curl -H "Authorization: Bearer …"` case. The tracker also gives up
  certainty the moment anything ambiguous happens (a control sequence it
  doesn't model, a screen it isn't driving), so it suggests nothing rather than
  something wrong. Stored per host, on the device, never synced.
