...
build_openssl () {
  ...
  (
    ...
    CC="$cc" \
    CFLAGS="-arch arm64 -isysroot $sysroot $minflag -O3" \
    ./Configure "$target" \
    ...
    # Setting CFLAGS at all replaces OpenSSL's own -O3, so the whole library —
    # X25519, the KEX, bcrypt_pbkdf, every packet — was built at -O0.
    grep -q -- "-O3" configdata.pm || {
      echo "ERROR: OpenSSL was configured without an optimization flag for $name;" >&2
      echo "       an explicit CFLAGS replaces its own -O3, so it would build at -O0." >&2
      exit 1
    }
...
build_libssh2 () {
  ...
  # CMAKE_BUILD_TYPE, not the `--config Release` below: "Unix Makefiles" is a
  # single-configuration generator and ignores --config entirely, so without
  # this libssh2 built with no optimization flags at all.
