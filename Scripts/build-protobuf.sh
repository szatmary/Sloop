...
build_slice () {
  ...
  # single-configuration generator and ignores --config, so without this every
  # object built unoptimized with `assert()` live. This library is merged whole
  # into libmosh.a, and 27 of the 41 objects that referenced `__assert_rtn`
  # there were these — an abort in any of them takes the whole app.
