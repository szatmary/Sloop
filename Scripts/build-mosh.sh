...
git -C mosh apply "$ROOT/Scripts/patches/mosh-tunnel-fd.patch"
grep -q "Connection( const char \*key_str, int tunnel_fd )" mosh/src/network/network.h \
  || { echo "tunnel-fd patch did not apply — upstream moved"; exit 1; }
