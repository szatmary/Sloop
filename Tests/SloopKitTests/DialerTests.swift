...
final class DialerTests: XCTestCase {
    ...
    /// A server reboot or a dropped path leaves an RST on the connection, and
    /// libssh2's next `send()` then raises SIGPIPE and takes the whole app
    /// down with it — Darwin has no `MSG_NOSIGNAL`, and libssh2 never sets
    /// this option itself. It has to be on the socket before anyone writes.
    ///
    /// Asserted on the option rather than by provoking a real SIGPIPE: the
    /// signal's disposition is process-wide, so a test process that survives
    /// the write would prove nothing about *this* socket.
    #if canImport(Darwin)
    func testDialedSocketIgnoresSIGPIPE() throws {
        // No accept() needed — the handshake completes into the listen backlog.
        let (listenFD, port) = makeListener()
        defer { close(listenFD) }
        ...
        XCTAssertEqual(getsockopt(fd, SOL_SOCKET, SO_NOSIGPIPE, &value, &length), 0)
        XCTAssertEqual(value, 1, "TCPDialer must set SO_NOSIGPIPE on the socket it returns")
    }
    #endif
