...
// A non-fatal exception, reported into the terminal instead of ending the
// session.
//
// Mosh exists to outlast exactly these: one datagram that fails integrity, an
// errno from recvmsg, an oversize packet, "no packet received" on a spurious
// wake. Upstream's stmclient prints and carries on; this brings that behaviour
// across the bridge.
//
// Repeats are dropped. A condition that recurs every pass through the loop —
// a socket erroring continuously — would otherwise fill the screen with the
// same line thousands of times a second.
static void mosh_notice(MoshSession *s, const std::string &text,
                        std::string &last_notice) {
  if (text == last_notice) return;
  last_notice = text;
  if (!s->on_output) return;
  const std::string line = "\r\n[sloop] mosh: " + text + "\r\n";
  s->on_output(line.data(), line.size(), s->ctx);
}
...
static void mosh_run_loop(MoshSession *s) {
  ...
  // The last non-fatal notice shown, so a recurring one is not repeated.
  std::string last_notice;
  ...
  try {
    ...
    while (s->running.load()) {
      // Each pass is its own try. Upstream stmclient puts the try *inside*
      // while(1) for this reason: a non-fatal exception must cost one packet,
      // not the session. Wrapping the whole loop meant a single stray datagram
      // — and a socket bound to :: on a globally routable cellular address
      // sees scanner traffic — ended the tab, left `start_shutdown()` uncalled,
      // and orphaned a `mosh-server` (which has no idle timeout by default)
      // along with its shell. Reconnect then started a second one.
      try {
        ...
        // Never fatal upstream, and not here: report it and keep the session.
        mosh_notice(s, std::string("network: ") + e.what(), last_notice);
        // The notice just wrote over a screen Mosh believes it owns, so the
        // next frame must be a full repaint rather than a diff against a
        // framebuffer that no longer matches what is displayed.
        initialized = false;
      } catch (const Crypto::CryptoException &e) {
        // `fatal` means the key or the session state is unusable; anything
        // else is one bad packet.
        if (e.fatal) throw;
        mosh_notice(s, std::string("crypto: ") + e.what(), last_notice);
        initialized = false;
      }
    }
  } catch (const Network::NetworkException &e) {
