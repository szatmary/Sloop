...
final class CommandSuggester {
    ...
    /// Feed the bytes arriving from the host, so the tracker can tell an
    /// echoed line from one typed at a prompt with echo off.
    ///
    /// Not optional wiring: without it nothing is ever confirmed as echoed and
    /// nothing is ever recorded. That is the right way round — a suggester
    /// wired up halfway learns nothing rather than learning passwords.
    func observeOutput(_ bytes: ArraySlice<UInt8>) {
        tracker.observeOutput(bytes)
    }
