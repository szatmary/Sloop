    ...
    private func wire(_ transport: Transport) {
        ...
        transport.onData = { [weak self, weak terminalView] bytes in
            DispatchQueue.main.async {
                ...
                // Same queue as `observeTyping`, which is what makes echo
                // confirmation meaningful: sent bytes and received bytes are
                // ordered against each other, so "did this come back?" has an
                // answer rather than a race.
                self?.suggester?.observeOutput(ArraySlice(bytes))
