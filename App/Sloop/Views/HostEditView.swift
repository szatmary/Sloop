...
struct HostEditView: View {
    ...
    /// Validates and stores a pasted key. Takes the raw fields rather than a
    /// built `NamedKey` because validating one — is this even a private key,
    /// does it need a passphrase, is that passphrase right — needs libssh2,
    /// which this view has no business importing.
    private let onSaveKey: (_ pem: String, _ name: String, _ passphrase: String?) throws -> Void
    ...
    init(host: SSHHost,
         ...
         onSaveKey: @escaping (_ pem: String, _ name: String, _ passphrase: String?) throws -> Void = { _, _, _ in },
                        ...
                        do {
                            ...
                            case .privateKey:
                                ...
                                if name.isEmpty {
                                    ...
                                    // The collision check lives with the store,
                                    // not here: it is the same policy the CLI
                                    // and every other import path enforce, and
                                    // two copies of it would drift.
                                    try onSaveKey(trimmedPastedPEM, name,
                                                  pastedPassphrase.isEmpty ? nil : pastedPassphrase)
                                }
