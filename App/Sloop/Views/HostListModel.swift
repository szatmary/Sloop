...
final class HostListModel: ObservableObject {
    ...
    /// Validate a pasted key and store it in the shared library.
    ///
    /// Routed through `KeyImport` like every other way a key gets in, so
    /// pasting something that is not a usable private key fails here, with a
    /// reason, instead of being stored and surfacing hours later as an
    /// authentication failure against a host.
    func saveLibraryKey(pem: String, name: String, passphrase: String?) throws {
        try importLibraryKey(Data(pem.utf8), name: name, passphrase: passphrase)
    }

    /// The one place the app stores a key, whatever brought the bytes in —
    /// paste, a file, or a pull from a host.
    @discardableResult
    func importLibraryKey(_ data: Data, name: String, passphrase: String?) throws -> NamedKey {
        #if canImport(CSSH)
        let key = try KeyImport.importKey(data, name: name, passphrase: passphrase, into: keys)
        refreshLibraryKeys()
        return key
        #else
        // No libssh2 in this build, so the key cannot be checked — and storing
        // an unchecked one is the failure this pipeline exists to remove. The
        // base project already tells the user SSH isn't built (see
        // TransportFactory); this is the same answer for keys.
        throw KeyImportError.libraryUnavailable
        #endif
    }

    /// Validates without storing, so a flow that must collect a passphrase can
    /// find out that it needs one.
    func inspectKey(_ data: Data, name: String, passphrase: String?) -> Result<NamedKey, KeyImportError> {
        #if canImport(CSSH)
        return KeyImport.prepare(data, name: name, passphrase: passphrase)
        #else
        return .failure(.libraryUnavailable)
        #endif
    }

    func removeLibraryKey(named name: String) throws {
        try keys.removeKey(named: name)
        refreshLibraryKeys()
    }

    /// An SFTP connection to `host`, for browsing `~/.ssh`.
    ///
    /// Its own connection rather than a live terminal session's: the key
    /// library has no business depending on whether a tab happens to be open,
    /// and the File Provider extension already establishes SFTP this way.
    /// The caller owns the returned client and must `close()` it.
    func sftpClient(for host: SSHHost) throws -> SFTPClient {
        let credential = try KeyLibrary.credential(for: host, keys: keys, credentials: credentials)
            ?? Credential()
        guard let knownHosts else {
            throw SloopStorage.StorageError.appGroupUnavailable(SloopStorage.appGroupIdentifier)
        }
        return try SFTPClientFactory.sftp(host: host,
