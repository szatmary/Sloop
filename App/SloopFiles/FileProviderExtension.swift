...
final class FileProviderExtension: NSObject, NSFileProviderReplicatedExtension {
    ...
    required init(domain: NSFileProviderDomain) {
        // A separate process from the app, so it needs its own call. Idempotent,
        // and this is the earliest hook the system gives an extension.
    ...
    func createItem(basedOn itemTemplate: NSFileProviderItem,
        ...
        do {
            ...
            service.perform { client, index in
                ...
                // `createFile`, not `write`: `write` replaces whatever is at
                // the path, so a file that appeared on the server since the
                // last listing was destroyed by a save from another app.
                // `options.mayAlreadyExist` says the system is retrying a
                // create of ours, which is the only case allowed to overwrite.
                let entry = try client.createFile(source, at: path,
                                                  mayAlreadyExist: options.contains(.mayAlreadyExist)) {
                    done, total in
                    guard total > 0 else { return }
                    progress.completedUnitCount = Int64(Double(done) / Double(total) * 100)
                }
                return try service.item(for: entry, client, index)
            } completion: { result in
                progress.completedUnitCount = 100
                switch result {
    ...
    func modifyItem(_ item: NSFileProviderItem,
        ...
        do {
            ...
            service.perform { client, index in
                ...
                // What the system asked for that did not happen. Empty is the
                // ordinary answer.
                var unapplied: NSFileProviderItemFields = []
                if changedFields.contains(.contents), let newContents {
                    do {
                        // Against the version the system says this edit was
                        // made from. Without the check, an edit made on the
                        // phone while offline overwrote whatever had happened
                        // on the server in between — a file edited in vim lost
                        // those changes the moment the phone came back.
                        try client.replaceFile(newContents, at: path,
                                               ifContentVersionMatches: version.contentVersion) {
                            done, total in
                            guard total > 0 else { return }
                            progress.completedUnitCount = Int64(Double(done) / Double(total) * 100)
                        }
                    } catch SFTPError.contentChanged {
                        // Reported rather than thrown. Saying "contents were
                        // not applied" and handing back the server's item is
                        // how the contract expresses a conflict: the system
                        // keeps the local edit and re-presents it against the
                        // newer copy. Failing the whole call would instead
                        // lose the rename that may have just succeeded above.
                        unapplied.insert(.contents)
                    }
                }
                ...
                try index.save()
                return (try service.item(for: try client.stat(path), client, index), unapplied)
            } completion: { result in
                progress.completedUnitCount = 100
                switch result {
                case .success(let (item, unapplied)):
                    // Sloop's own writes need not wait for the next
                    // enumeration; only remote changes do.
                    service.signalChange()
                    completionHandler(item, unapplied, false, nil)
                case .failure(let error):
                    completionHandler(nil, [], false, FileProviderError.from(error))
                }
            }
