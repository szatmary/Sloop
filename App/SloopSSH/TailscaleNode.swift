...
public final class TailscaleNode: @unchecked Sendable {
    ...
    func dial(host: String, port: Int) throws -> Int32 {
        ...
        // tsnet hands back a real fd, so it needs the same protection as one we
        // opened ourselves — the Go runtime's own SIGPIPE handling covers Go's
        // writes, not libssh2's.
        do {
            try ignoreSIGPIPE(on: conn)
        } catch {
            close(conn)
            throw error
        }
