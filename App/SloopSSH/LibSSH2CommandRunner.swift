...
final class LibSSH2CommandRunner: CommandRunner {
    ...
    /// Where the last run connected. Captured while the connection is still
    /// open, because `execute` closes it before anyone could ask.
    ///
    /// Written on the exec thread and read from the completion that same
    /// thread invokes, but locked anyway: the property is reachable from
    /// wherever the caller keeps the runner.
    private let addressLock = NSLock()
    private var lastPeerAddress: String?
    ...
    private func execute(_ command: String) -> Result<CommandResult, Error> {
        ...
        // While the socket is still open: the Mosh bootstrap needs to send its
        // UDP packets to this machine, not to whatever DNS answers next.
        addressLock.lock()
        lastPeerAddress = connection.peerAddress
        addressLock.unlock()
