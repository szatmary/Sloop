...
final class LibSSH2Transport: Transport, SessionCommandRunner {
    ...
    init(host: SSHHost,
        ...
        // Lets `close()` reach the phases that run before the event loop —
        // handshake, auth, channel open, the PTY and shell requests — which
        // otherwise could not be stopped at all. Weak, because the connection
        // is owned by this transport; a transport that has gone away counts as
        // closed.
        connection.isCancelled = { [weak self] in self?.isClosing ?? true }
    }
    ...
    /// How long the shell channel may spend closing politely.
    ///
    /// The same bounded trade the forwarded agent makes: a peer that never
    /// acknowledges must not decide when teardown ends. Short, because nothing
    /// waits on it but the thread that is already finished.
    static let channelCloseTimeout: TimeInterval = 2
    ...
    /// What the event loop should do with the result of a channel read.
    enum ChannelReadOutcome: Equatable {
        /// Bytes are waiting in the buffer.
        case data(Int)
        /// Nothing to read at this instant; go round again.
        case wouldBlock
        /// The remote closed the channel. The session is over and nothing is
        /// wrong.
        case endOfFile
        /// The connection died underneath us, carrying libssh2's code.
        case failed(Int)
    }

    /// Classifies one `libssh2_channel_read_ex` result.
    ///
    /// Pulled out of the loop because the loop needs a live server to run, and
    /// that is precisely how the bug survived: every test passed while a dead
    /// socket span at 100% CPU. The rule that was missing is the last one — a
    /// TCP-level death never sets channel EOF, because the channel never got to
    /// close politely, so "error but no EOF" was read as "nothing to read" and
    /// the loop polled a closed fd forever.
    static func readOutcome(n: Int, isEOF: Bool) -> ChannelReadOutcome {
        if n > 0 { return .data(n) }
        // EOF first: a remote that hung up mid-read is still a clean goodbye,
        // and saying so beats reporting whatever code came with it.
        if isEOF { return .endOfFile }
        if n == 0 || n == Int(LIBSSH2_ERROR_EAGAIN) { return .wouldBlock }
        return .failed(n)
    }

    /// The session died mid-flight. libssh2's own description is the only thing
    /// that distinguishes "server rebooted" from "the path went away".
    private func transportFailure(_ code: Int) -> Error {
        SSHError.connectionFailed("\(connection.lastError) [libssh2 \(code)]")
    }
    ...
    private func run() {
        ...
        defer {
            connection.retryDuringTeardown(within: Self.channelCloseTimeout) {
        ...
        finish(eventLoop(session: session, channel: channel))
    }
    ...
    /// Pumps the session until it ends. Returns the error that ended it, or
    /// nil for a clean close — either the caller asking, or the remote hanging
    /// up. It used to return nothing at all, so `run()` reported every death as
    /// a clean exit and the tab stayed "Connected".
    private func eventLoop(session: OpaquePointer, channel: OpaquePointer) -> Error? {
        ...
        while true {
            ...
            if !pending.isEmpty {
                ...
                pending.withUnsafeBytes { raw in
                    ...
                    while offset < pending.count {
                        ...
                        // Not a short write to retry: the connection is gone.
                        // Requeueing here is what made every keystroke after a
                        // server reboot retry forever against a dead socket.
                        if n < 0 { writeFailure = n; return }
                        offset += n
                    }
                }
                if let writeFailure { return transportFailure(writeFailure) }
            ...
            while draining {
                ...
                switch Self.readOutcome(n: n, isEOF: libssh2_channel_eof(channel) != 0) {
                case .data(let count):
                    onData?(ArraySlice(buffer[0..<count]))
                    readData = true
                case .wouldBlock:
                    draining = false
                case .endOfFile:
