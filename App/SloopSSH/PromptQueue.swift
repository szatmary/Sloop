...
/// against itself.
/// Public because the prompters that use it live in the app while the code
/// that blocks on an answer lives here: one queue, so a host-key prompt and a
/// signature prompt take turns rather than both trying to present at once.
public final class PromptQueue {
    ...
    public static let shared = PromptQueue()
    ...
    public func request(present: @escaping (@escaping (Bool) -> Void) -> Void) -> Bool {
