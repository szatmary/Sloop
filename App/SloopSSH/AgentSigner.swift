...
final class AgentSigner {
    ...
    private static func identity(session: OpaquePointer, key: NamedKey) -> AgentIdentity? {
        ...
        let rc = key.privateKeyPEM.withCString { pemPtr in
            withPassphraseCString(key.passphrase) { passPtr in
    ...
    /// plain `const char *` on `_libssh2_pub_priv_keyfilememory`, so the
    /// rebinding happens here; `withPassphraseCString` still owns the rule that
    /// a missing passphrase is "" and never NULL, and why.
