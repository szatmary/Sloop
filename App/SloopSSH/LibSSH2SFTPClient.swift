...
final class LibSSH2SFTPClient: SFTPClient, @unchecked Sendable {
    ...
    func lstat(_ path: String) throws -> SFTPEntry {
        ...
        lock.lock(); defer { lock.unlock() }
        let sftp = try subsystem()
        // The one caller that must not follow a link is `removeRecursively`:
        // a symlink to a directory answers STAT as a directory, and the walk
        // then descends through it and unlinks the target's contents.
    ...
    func readData(_ path: String, maximumBytes: Int) throws -> Data {
        ...
        // Checked before opening the handle where the server reports a size, so
        // an oversized file costs no transfer at all. Servers that report none
        // are caught by the running total below instead — the cap holds either
        // way, which is why it is enforced twice rather than trusting `stat`.
        let attributes = try statLocked(path, on: sftp, kind: LIBSSH2_SFTP_STAT)
        if let reported = Self.reportedSize(of: attributes), reported > Int64(maximumBytes) {
            throw SFTPError.tooLarge(path, limit: maximumBytes)
        }

        return try withHandle(path, on: sftp, flags: UInt(LIBSSH2_FXF_READ), mode: 0,
                              kind: LIBSSH2_SFTP_OPENFILE) { handle in
            var contents = Data()
            var buffer = [UInt8](repeating: 0, count: Self.chunkSize)
            while true {
